...
//#include <stdio.h>
#include "68000.h"
#include "op68k.h"      // for protected registers
#include "st.h"
#ifdef DEBUG
#include <assert.h>
#endif
...
uint8           vid_syncmode;
uint8           vid_shiftmode;
uint8           vid_oldsyncmode;

uint16          dma_car, dma_scr, dma_sr, dma_mode;
uint8           _dma_padding0, dma_adrh, dma_adrm, dma_adrl;

uint8           mfp_gpip, mfp_aer, mfp_ddr, mfp_iera;
uint8           mfp_ierb, mfp_ipra, mfp_iprb, mfp_isra;
uint8           mfp_isrb, mfp_imra, mfp_imrb, mfp_ivr;
uint8           mfp_tacr, mfp_tbcr, mfp_tcdcr, mfp_scr;
uint8           mfp_ucr, mfp_rsr, mfp_tsr, mfp_udr;

uint8           _iomisc_padding[3];
uint8           memconf = 0;

int32           mfp_reg[12];

uint8           acia2_sr, acia2_dr, _acia_padding[2];
uint8           acia1_cr, acia1_sr, acia1_dr, acia2_cr;

uint16          blt_halftone[16];
int16           blt_src_x_inc, blt_src_y_inc;
uint32          blt_src_addr;
int16           blt_end_3,_blt_padding0;
int16           blt_end_1, blt_end_2;
int16           blt_dst_x_inc, blt_dst_y_inc;
uint32          blt_dst_addr;
uint16          blt_x_cnt, blt_y_cnt;
int8            _blt_padding1[3];
int8            blt_skew;
int8            blt_ready, blt_hop, blt_op, blt_status;

uint8			psg[16];
uint8           _psg_padding[2];
uint8           psgcontrol;
uint8           psgmode;
uint32			psg_reg_vol, psg_cur_reg_vol;
uint32          phase0, phase1, phase2, phase3, phase4;
uint32          psg_epos, nrand, sampos;

uint8 sample[64*1024] = {0};
int samvol2[4096] = {0};
int samvol[16] = {0,0,0,1,1,1,2,3,5,7,11,17,25,38,57,85};
...
{
    //DBG("IOWB: %08x %04x", address, value);
	...
	if (address<0x800) {
		...
		case VID_SYNCMODE&0x7fff:
            ...
            /*
			if (hbl==263){
				if (vid_oldsyncmode==0&&vid_syncmode==2){
					end_visible_screen=264+38;
					vid_oldsyncmode=-2;
				}else 	vid_oldsyncmode=vid_syncmode;
			}else vid_oldsyncmode=-1;
            */
			break;
		case VID_SHIFTMODE&0x7fff:
			vid_shiftmode = value;
            vid_flag |= VIDFLAG_REZ;
			break;
	...
	if (address<0x900) {
        ...
        else if (address==2)
        {
            ...
            const uint8 bitmask[16] = {0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x1f,0x1f,0x1f,0xff,0xff,0xff,0xff,0xff};
            value &= bitmask[psgcontrol];
            psg[psgcontrol] = value;

            #if defined(ATARI)
            if (psgmode & PSGMODE_DIRECT) {
                if (psgcontrol < 14) {
                    if (psgcontrol == 7) {
                        value |= 0x40;
                    }
                    *((volatile uint8*)0xff8800) = psgcontrol;
                    *((volatile uint8*)0xff8802) = value;
                }
            } else if (psgmode & PSGMODE_XBIOS) {
                if (psgcontrol < 14) {
                    if (psgcontrol == 7) {
                        value |= 0x40;
                    }
                    Giaccess(value, 0x80 | psgcontrol);    
                }
            } else
            #endif
            {
                switch (psgcontrol) {
                    ...
                    case 9:
                    case 10:
                        psg_cur_reg_vol=psgcontrol;
                        if ((psgcontrol==psg_reg_vol) && (psgmode == PSGMODE_EMULATED)) {
                            sample[sampos++]=samvol[psg[8]]+samvol[psg[9]]+samvol[psg[10]];
                        }
                        break;
                    break;
                    case 12:
...
{
    //DBG("IOWW: %08x %04x", address, value);
    if (address >= VID_COL0 && address <= VID_COL15)
    {
        uint32 idx = ((address - VID_COL0) >> 1) & 0xf;
        value &= 0x0777;
        vid_col[idx] = value;
        if (vid_rasters) {
            uint32 line = 0;
            if (vid_rasters && ((vid_hbl >= 64) && vid_hbl < (199+64))) {
                line = vid_hbl - 63;
            }
            vid_colr[(line << 4) + idx] = value;
            vid_colf[line] |= (1 << idx);
        }
        vid_flag |= VIDFLAG_PALETTE;
		return;
	}
	else
        switch (address) {
        ...
        case DMA_CAR:
            if (dma_mode & 0x10) dma_scr = value;
				...
				case 0:
					fdc_command = value;
					if (!fdc_transfert||((fdc_command & 0xf0) == 0xd0)) FDCCommand ();
					break;
				case 2:
					fdc_track = value;
					break;
...
uint8 DoIORB(uint32 address)
{
	...
	if (address<0x800) {
		...
		/*	if (hbl<64){
				return 0;
			}else if (hbl<end_visible_screen){
				if ((cur_vid_scan>=96)&&(cur_vid_scan<96+320)) return (unsigned char)((vid_baseh<<16)+(vid_basem<<8)+(hbl-63)*160+(((512-hsync)+cycleco)>>1));
	
				else if (cur_vid_scan<96)	return (unsigned char)((vid_baseh<<16)+(vid_basem<<8)+(hbl-63)*160);
				else if (cur_vid_scan>=96+320)	return (unsigned char)((vid_baseh<<16)+(vid_basem<<8)+(hbl-63)*160+160);
			}else{
				adr_vid=(vid_baseh<<16)+(vid_basem<<8)+(end_visible_screen-64)*160;
				return (unsigned char)adr_vid;
			}*/
