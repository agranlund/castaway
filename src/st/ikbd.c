...
#include "config.h"
#include "st.h"
#define IBS 16                  /* Ikbd input buffer size */
#define OBS 32                  /* Ikbd output buffer size */
...
struct _joy     joystick;
struct _mouse   mouse;
unsigned char   ikbd_pause;
unsigned char   TempButtons;
...
{
    //DBG("IkbdRecv: %d", inchar);
    if (inbuffi == IBS) {
        //DBG("IkbdRecv: Input buffer overrun!");
        inbuffi = 0;
    }
    ...
    case 0x0e:                  /* load mouse position */
        ...
        if (inbuffi == 6) {
            inbuffi = 0;
            mouse.x = mouse.stx = (inbuff[2] << 8) | inbuff[3];
            mouse.y = mouse.sty = (inbuff[4] << 8) | inbuff[5];
        }
        break;
    ...
    case 0x16:                  /* joystick interrogation */
        ...
        inbuffi = 0;
        IkbdSend (0xfd);
        IkbdSend (joystick.state[0]);
        IkbdSend (joystick.state[1]);
        break;
    ...
    case 0x80:                  /* reset */
        if (inbuffi == 2) {
            ...
            case 0:
                mouse.x = mouse.stx = 160;
                mouse.y = mouse.sty = 100;
                break;
            case 1:
                mouse.x = mouse.stx = 320;
                mouse.y = mouse.sty = 100;
                break;
            case 2:
                mouse.x = mouse.stx = 320;
                mouse.y = mouse.sty = 200;
                break;
            }
        }
        break;
...
if (joystick.mode==4) IkbdJoystick();
	*/
    if (outbuffi>0 /*&& !ikbd_pause*/ && (acia1_sr & 0x1) == 0) {
		...
		acia1_dr = outbuff[0];
        memmove(&outbuff[0], &outbuff[1], (OBS - 1));
        outbuffi--;
    }
}
...
void IkbdJoystickEvent (unsigned char idx, int joystate)
{
	//Has joystick moved?
    idx &= 1;
	if (joystate == joystick.state[idx])
        return;

    if ((joymask & (1 << idx)) == 0) {
        joystick.state[idx] = 0;
        return;
    }

    joystick.state[idx]=joystate;

	//send fire button to mouse as well
	if ((mouse.mode==1 || mouse.mode==0) && (idx == 1)) {
		if (joystate&0x80) {
			IkbdSend (0xf9);
            IkbdSend (0);
            IkbdSend (0);
...
void IkbdLoop()
{
	...
	if (joystick.mode==4) {
		...
		joytick+=2;
		if (joytick<joystick.rate)
            return;
		joytick=0;
		IkbdSend ((joystick.state[1]>>7) | ((joystick.state[0]>>6)&2));
		IkbdSend ((joystick.state[1]&15) | ((joystick.state[0]<<4)));
	}
}
