...
static int write_head(int fd, const struct aexec *a)
{
	...
	unsigned long prgFlags = a->a_AZero2;
	if (singletask)
	{
		prgFlags |= 0x10000;	// FreeMiNT: singletasking application
		prgFlags |= 0x10;		// Memory protection mode: Global
	}
/*
    prgFlags &= ~1;     // Get rid of fastload flag
    prgFlags |= 0x2;    // May execute from fastram
    prgFlags |= 0x4;    // May allocate from fastram
*/
